...
/// ``InserterOverridesStore/update(_:)``: read, change and write as one step on the store.
extension InserterOverridesStoreTests {
    @Test func updateChangesOnlyWhatTheClosureChanges() async throws {
        ...
        try await store.save(InserterOverrides(methods: ["com.apple.Notes": .paste, "com.apple.Terminal": .accessibility]))
        ...
        let expected = InserterOverrides(methods: ["com.apple.Terminal": .accessibility, "com.barebones.bbedit": .paste])
    ...
    @Test func updateStartsFromNothingWhenTheFileIsMissing() async throws {
        ...
        #expect(try await store.load() == InserterOverrides(methods: ["com.apple.Notes": .paste]))
        let attributes = try FileManager.default.attributesOfItem(atPath: folder.fileURL.path)
        #expect(attributes[.posixPermissions] as? Int == 0o600)
    }

    /// Like ``InserterOverridesStore/load()``: the damaged file is kept for the user, and the
    ...
    @Test func updateSetsACorruptFileAsideBeforeWriting() async throws {
        ...
        #expect(updated == InserterOverrides(methods: ["com.apple.Notes": .paste]))
